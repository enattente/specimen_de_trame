function init_interface() {
  ...
  deepInput.addEventListener('input', () => {
   ...
   const profondeurMemoireInput = document.getElementById('profondeur-memoire-input');
   profondeurMemoireInput.value = profondeur_memoire;
   profondeurMemoireInput.addEventListener('input', () => {
     profondeur_memoire = parseInt(profondeurMemoireInput.value);
     registre_area_visited.update_profondeur_memoire(profondeur_memoire);
   });

  // Nombre de règles
  const nbValueInput = document.getElementById('nb-value-input');
  ...
  // Particules min
  const nbParticulesMinInput = document.getElementById('nb-particules-min-input');
  nbParticulesMinInput.value = nb_particules_min;
  nbParticulesMinInput.addEventListener('input', () => {
    nb_particules_min = parseInt(nbParticulesMinInput.value);
    rules_nb_particules.update_intervalle_rules(nb_particules_max, nb_particules_min);
  });

  // particule height
  const particleHeightInput = document.getElementById('particle-height-input');
  particleHeightInput.value = particule_pixel_height;
  particleHeightInput.addEventListener('input', () => {
    particule_pixel_height = parseInt(particleHeightInput.value);
  });

  // particule width
  const particleWidthInput = document.getElementById('particle-width-input');
  particleWidthInput.value = particule_pixel_width;
  particleWidthInput.addEventListener('input', () => {
    particule_pixel_width = parseInt(particleWidthInput.value);
  });

  // noise angle level
  const noiseAngleLevelInput = document.getElementById('noise-angle-level-input');
  noiseAngleLevelInput.value = noise_angle_level;
  noiseAngleLevelInput.addEventListener('input', () => {
    noise_angle_level = parseFloat(noiseAngleLevelInput.value);
    Particule.setNoiseAngleLevelAll(noise_angle_level);
  });
  ...
  // rotation des particules brightness
  const rotationParticulesBrightnessInput = document.getElementById('rotation-particules-brightness-btn');
  rotationParticulesBrightnessInput.addEventListener('click', () => {
    // Effacer le buffer, pas le canvas principal
    buffer.background(255);
    
    Particule.get_all_particules().forEach(particule => {
      // Ne plus passer noise_angle_level car la méthode n'en a plus besoin
      particule.rotate_brightness(map_image_brightness);
      particule.draw_rectangle(buffer);
    });
    
    // Afficher le buffer mis à jour sur le canvas principal
    image(buffer, 0, 0, width_buffer_in_canva, height_buffer_in_canva);
  });

  // rotation des particules detail
  const rotationParticulesDetailInput = document.getElementById('rotation-particules-detail-btn');
  rotationParticulesDetailInput.addEventListener('click', () => {
    // Effacer le buffer, pas le canvas principal
    buffer.background(255);
    
    Particule.get_all_particules().forEach(particule => {
      // Ne plus passer noise_angle_level car la méthode n'en a plus besoin
      particule.rotate_detail(map_image_detail);
      particule.draw_rectangle(buffer);
    });
  ...
  // rotation des particules noise
  const rotationParticulesNoiseInput = document.getElementById('rotation-particules-noise-btn');
  rotationParticulesNoiseInput.addEventListener('click', () => {
    // Effacer le buffer, pas le canvas principal
    buffer.background(255);


    Particule.get_all_particules().forEach(particule => {
      // Passer la valeur noise_angle_level actuelle à la méthode
      particule.rotate_noise(noise_angle_level);
      particule.draw_rectangle(buffer);
    });
  ...
  //////////////////////////////////////////////////////////////
  // controle parmatere exploration
  //////////////////////////////////////////////////////////////
  
  
  // bouton pour mettre à jour l'itinéraire
  updateItineraireBtn = document.getElementById('itineraires-update-btn');
  updateItineraireBtn.addEventListener('click', () => {
    update_itineraires_from_text();
  });

  // Bouton pour mettre à jour le nombre d'enfants
  updateNbChildsBtn = document.getElementById('nb_childs-update-btn');
  updateNbChildsBtn.addEventListener('click', () => {
    update_nb_childs_from_text();
  });

    // Bouton pour mettre à jour le nombre d'enfants
  updateDepartBtn = document.getElementById('depart-update-btn');
  updateDepartBtn.addEventListener('click', () => {
  ...
  // Bouton reset les reègles par défaut
...
//////////////////////////////////////////////////////////////
// fonction de formattage texte 
//////////////////////////////////////////////////////////////

function ecrire_interface_itineraires() {
  let itineraire_list = navigation.get_itineraires();
  let formattedText = itineraire_list
      .map(it => `${it.x},${it.y}`)
      .join('\n');
  document.getElementById('itineraires-liste').value = formattedText;
}


function update_itineraires_from_text() {
  let text = document.getElementById('itineraires-liste').value;
  ...
  for (let line of lines) {
      ...
      destinations_x.push(parts[0]);
      destinations_y.push(parts[1]);
      } else {
      alert(`Ligne invalide : "${line}". Elle doit contenir exactement 2 valeurs numériques.`);
      return;
      }
  }
  navigation.update_itineraires(destinations_x, destinations_y);
  console.log('Itinéraires mis à jour avec succès !');
}


function ecrire_interface_nb_childs() {
  let nb_childs_list = navigation.get_nb_child();
  let formattedText = nb_childs_list
      .map(it => `${it.childs},${it.proba}`)
      .join('\n');
  document.getElementById('nb_childs-liste').value = formattedText;
}


function update_nb_childs_from_text() {
  let text = document.getElementById('nb_childs-liste').value;
  let lines = text.trim().split('\n');
  ...
  navigation.update_nb_child(nb_childs, probas);
  console.log('Itinéraires mis à jour avec succès !');
}

function ecrire_interface_depart() {
if (!departs || !Array.isArray(departs)) return;

let formattedText = departs.map(row => row.join('')).join('\n');
document.getElementById('depart-liste').value = formattedText;
}



function update_depart_from_text() {
let text = document.getElementById('depart-liste').value;
let lines = text.trim().split('\n');

departs = lines.map(line =>
