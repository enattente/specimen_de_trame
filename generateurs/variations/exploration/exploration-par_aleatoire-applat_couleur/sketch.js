  ...
  for (let i = 0; i < nb_lignes+1; i++) {
    for (let j = 0; j < nb_colonnes+1; j++) {
      if (departs[i][j] === 1) {
        ...
        console.log("value_brightness", value_brightness);
  ...
  if (img_init !==-1 && img_init <deep) {
    ...
    for (let i = 0; i < parents.length; i++) {
      ...
      for (let j = 0; j < nb_child_parent; j++) {
        let destination = navigation.get_itineraire_result_detail(x_parent, y_parent, map_image_aleatoire);
        ...
        child.draw_on_buffer_applat_couleur(buffer);
      }
    }
    registre_generations.next_generation(); 
    img_init++;
