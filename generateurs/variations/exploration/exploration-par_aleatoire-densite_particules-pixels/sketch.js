...
let values_map_nb_aleatoire = [];

let atlas_map_brightness = [];
let atlas_map_detail = [];
let atlas_map_nb_aleatoire = [];
...
function setup() {
  ...
  map_image_aleatoire = new MapImage({nb_value:nb_value_aleatoire, valeur_source_min:0, valeur_source_max:125});
  ...
  for (let i = 0; i < nb_lignes+1; i++) {
    for (let j = 0; j < nb_colonnes+1; j++) {
      if (departs[i][j] === 1) {
        ...
        portion.draw_on_buffer_particule_pixels(buffer);
  ...
  if (img_init !==-1 && img_init <deep) {
    ...
    for (let i = 0; i < parents.length; i++) {
      ...
      for (let j = 0; j < nb_child_parent; j++) {
        let destination = navigation.get_itineraire_result_detail(x_parent, y_parent, map_image_detail);
        ...
        child.draw_on_buffer_particule_pixels(buffer);
      }
    }
    registre_generations.next_generation(); 
    img_init++;
