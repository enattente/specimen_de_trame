...
function setup() {
  ...
  map_image_detail = new MapImage({nb_value:nb_value_colors, valeur_source_min:0, valeur_source_max:125});
...
function init_image() {
  ...
  // Initialiser values_map_detail
  values_map_detail = [];
  map_image_detail.make_map_detail(img, stepsize_width, stepsize_height);
  values_map_detail = map_image_detail.get_value_map();
  ...
  if (img_init !== -1) {
    if (img_init < liste_etapes.length - 1) {
      for (let i = liste_etapes[img_init]; i < liste_etapes[img_init + 1]; i++) {
        ...
        let value_brightness = values_map_brightness[y][x]; // 
        let value_detail = values_map_detail[y][x]; // 
        let nb_particules = rules_nb_particules.get_rule(value_brightness);
        ...
        c.draw_on_buffer_rectangle_oriente(buffer);
      }
