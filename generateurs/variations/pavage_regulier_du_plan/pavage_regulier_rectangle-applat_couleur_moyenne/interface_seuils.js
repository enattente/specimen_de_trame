function init_interface_seuil() {
  ...
  p.setup = () => {
    ...
    niveaux_brightness = new Niveau(p, niveaux_x_left, niveaux_y_top, niveaux_x_right, niveaux_y_bottom, rules_brightness, map_image_brightness);
    niveaux_brightness.update_niveaux();

    miniatures_brightness = new Miniatures(p, miniatures_x_left, miniatures_y_top, miniatures_x_right, miniatures_y_bottom, nb_value_colors, map_image_brightness, rules_brightness, slider_brightness);
  ...
  p.mouseDragged = function() {
    ...
    if (selected_niveau !== -1) {
      ...
      let value = p.map(y_clamped, niveaux_y_top, niveaux_y_bottom, rules_brightness.rules_max, rules_brightness.rules_min);
      value = p.int(value);
      rules_brightness.update_one_rule(selected_niveau, value)  
    }
