function init_interface_seuil() {
  ...
  p.setup = () => {
    ...
    // definir les objet 
    slider_brightness = new SliderBar(p, slider_x_start, slider_y_start, slider_x_end, slider_y_end, map_image_brightness);
    slider_brightness.update_sliders();

    niveaux_brightness = new Niveau(p, niveaux_x_left, niveaux_y_top, niveaux_x_right, niveaux_y_bottom, rules_state_vertical, map_image_brightness);
    niveaux_brightness.update_niveaux();

    niveaux_2 = new Niveau(p, niveaux_x_left_2, niveaux_y_top_2, niveaux_x_right_2, niveaux_y_bottom_2, rules_division, map_image_brightness);
    niveaux_2.update_niveaux();

    miniatures_brightness = new Miniatures(p, miniatures_x_left, miniatures_y_top, miniatures_x_right, miniatures_y_bottom, nb_value_colors, map_image_brightness, rules_state_vertical, slider_brightness);
  ...
  p.mousePressed = function() {
    ...
    let niveaux_positions_2 = niveaux_2.get_position();
    ...
    for (let i = 0; i < niveaux_positions_2.length ; i++) {
      let x = niveaux_positions_2[i].x;
      let largeur = niveaux_positions_2[i].largeur;
      if (
  ...
  p.mouseDragged = function() {
    ...
    if (selected_slider !== -1) {
      ...
      let value = p.map(x_clamped, slider_x_start, slider_x_end, 0, 255);
      value = p.int(value);
      map_image_brightness.change_one_seuil(selected_slider, value);
    }

    if (selected_niveau !== -1) {
      let y_clamped = p.constrain(p.mouseY, niveaux_y_top, niveaux_y_bottom);
      let value = p.map(y_clamped, niveaux_y_top, niveaux_y_bottom, rules_state_vertical.rules_max, rules_state_vertical.rules_min);
      value = p.int(value);
      rules_state_vertical.update_one_rule(selected_niveau, value)  
    }
    ...
    niveaux_2.update_niveaux();
    if (img) {
      miniatures_brightness.update_red();
