...
function setup() {
  ...
  rules_division = new Rules({map:map_image_detail, rules_max:0, rules_min:deep+1});


  rules_detail = new Rules({map:map_image_detail, rules_max:125, rules_min:0});
...
function init_image() {
  ...
  for (let y = 0; y < initial_map_color.value_map.length; y++) {
    for (let x = 0; x < initial_map_color.value_map[y].length; x++) {
      ...
      let color = rules_colors.get_rule(brightness_value);
      let divisions = rules_division.get_rule(detail_value); 
      let detail = rules_detail.get_rule(detail_value);
          ...
          if (parent.divisions > img_init) {
            ...
            for (let pos of positions) {
              ...
              if (map_y < color_map_info.value_map.length && map_x < color_map_info.value_map[map_y].length) {
                ...
                enfant.draw_on_buffer_particule_pixels(buffer);
              }
            }
          }
        }
